...
bool CheckStakeProtocolV2(uint256 nStakeModifierV2, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, CAmount nValueIn, const uint256& bnTarget,
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (nTxTime >= Params().NewKernelProtocolTime()) {
        // New protocol
        if (!CheckStakeNewProtocol(stake->GetUniqueness(), stake->GetValue(), nStakeModifier, bnTargetPerCoinDay, nBlockFromTime,
        ...
        // Old protocol
        if (!CheckStakeProtocolV2(nStakeModifierV2, txPrev, txin.prevout, tx.nTime, stake->GetValue(), bnTargetPerCoinDay,
                        hashProofOfStake)) {
            return error("CheckProofOfStake() : INFO: check kernel failed on coinstake %s, hashProof=%s \n",
                        tx.GetHash().GetHex(), hashProofOfStake.GetHex());
        }
